...
INFINIOP_CUDA_KERNEL softmaxCuda(
    ...
    softmaxKernel<BLOCK_SIZE, Tdata, Tcompute>(y, x, batch, height, width, y_stride_b, y_stride_h, x_stride_b, x_stride_h);
}
...
infiniStatus_t Descriptor::create(
    ...
    auto info = SoftmaxInfo::create(y_desc, x_desc);
    CHECK_RESULT(info);
...
infiniStatus_t launchKernel(void *y, const void *x, infiniDtype_t dtype,
    ...
    if (dtype == INFINI_DTYPE_F16) {
        softmaxCuda<BLOCK_SIZE, half, float>
        ...
        softmaxCuda<BLOCK_SIZE, __nv_bfloat16, float>
            <<<grid, BLOCK_SIZE, 0, stream>>>((__nv_bfloat16 *)y, (const __nv_bfloat16 *)x,
                                              batch_size, seq_len, total_seq_len,
                                              y_stride_b, y_stride_i,
                                              x_stride_b, x_stride_i);
    } else if (dtype == INFINI_DTYPE_F32) {
        softmaxCuda<BLOCK_SIZE, float, float>
